...
DEFAULT = {
    # Scale-ready defaults (~350M–1.3B configurable)
    "n_layers": 24,
    "n_heads": 16,
    "d_model": 1536,
    "d_ff": 6144,
    ...
    "clip_grad": 1.0,
    "use_wandb": False,
    "wandb_project": "gpt3_5_style_train",
    "wandb_run_name": None,
    # Precision / Orchestration
    "amp_dtype": "bf16",  # "fp16" or "bf16"
    "use_fsdp": False,
    "fsdp_min_params": 1e6,
    "use_deepspeed": False,
    # Data
    "use_webdataset": False,
    "webdataset_url": "",  # e.g., "s3://bucket/train-{00000..09999}.tar"
    # RLHF defaults
    "rlhf_enabled": False,
    "rm_lr": 1e-4,
    "ppo_lr": 1e-5,
...
def deepspeed_config_from_cfg(cfg: Dict[str, Any]) -> Dict[str, Any]:
    # Minimal ZeRO-3 config; tweak per cluster
...
def sft_train_loop(model: nn.Module, train_dl: DataLoader, val_dl: Optional[DataLoader],
    ...
    if os.path.exists(model_path):
        ...
        if "optimizer" in ckpt and ckpt["optimizer"] is not None and engine is None:
                ...
                if accum >= grad_accum or engine is not None:
                    ...
                    if is_main_process():
                        if progress:
                            ...
                            lr = optim.param_groups[0]["lr"] if engine is None else engine.optimizer.param_groups[0]["lr"]
                        ...
                        if writer and step % config["log_every"] == 0:
                            ...
                            writer.add_scalar("train/lr", optim.param_groups[0]["lr"] if engine is None else engine.optimizer.param_groups[0]["lr"], step)
                        if wandb and config.get("use_wandb", False) and step % config["log_every"] == 0:
                            wandb.log({"train/loss": float(loss.item() * (grad_accum if engine is None else 1)),
                                       "train/lr": optim.param_groups[0]["lr"] if engine is None else engine.optimizer.param_groups[0]["lr"],
                                       "step": step})
                    ...
                    if is_main_process() and (step % config["save_every"] == 0 or step >= total_steps):
                        ckpt = {
                            ...
                            "optimizer": None if engine is not None else optim.state_dict(),
...
def main():
    ...
    if args.cmd == "convert_hf":
        ...
        cfg = {
            ...
            "total_steps": args.total_steps, "save_every": args.save_every, "device": DEFAULT["device"],
            "vocab_size": DEFAULT["vocab_size"], "use_checkpoint": args.use_checkpoint, "seed": args.seed,
