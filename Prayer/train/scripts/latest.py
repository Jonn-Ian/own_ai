...
import os, json, math, shutil, argparse, hashlib, random, time
...
# Optional libs
try:
    from sentencepiece import SentencePieceProcessor, SentencePieceTrainer  # type: ignore
except Exception:
...
DATASET_DIR   = "drive/MyDrive/Colab/assets/jsonl"
SAVE_DIR      = "drive/MyDrive/Colab/assets/weights"

# Intermediate processing directories
RAW_DATA_DIR   = "drive/MyDrive/Colab/assets/raw"
PROCESSED_DIR  = "drive/MyDrive/Colab/assets/processed"
TOKENS_DIR     = "drive/MyDrive/Colab/assets/tokens"
SPM_PREFIX     = "drive/MyDrive/Colab/assets/tokens/spm"
...
DEFAULT = {
  "n_layers": 12, "n_heads": 8, "d_model": 768, "d_ff": 3072,
  "dropout": 0.1, "max_seq_len": 512,
  "batch_size": 8, "grad_accum_steps": 8,
  "lr": 2e-4, "weight_decay": 0.1, "betas": (0.9, 0.95),
  "warmup_steps": 1000, "total_steps": 50000,
  "save_every": 2000, "use_checkpoint": True, "seed": 42,
  "log_every": 100, "eval_every": 2000, "clip_grad": 1.0,
  "use_wandb": False, "wandb_project": "gpt3_5_style_train", "wandb_run_name": None,
  "amp_dtype": "bf16", "use_fsdp": False, "fsdp_min_params": 1e6,
  "use_deepspeed": False, "use_webdataset": False, "webdataset_url": "",
  # RLHF defaults
  "rlhf_enabled": False, "rm_lr": 1e-4, "ppo_lr": 1e-5,
  "ppo_epochs": 1, "ppo_batch_size": 8, "ppo_kl_coef": 0.02,
  "ppo_clip": 0.2, "rm_train_steps": 10000,
}
...
def convert_hf_dataset(dataset_name="daily_dialog", out_dir=RAW_DATA_DIR):
    ...
    for split in ds.keys():
        ...
        with out_file.open("w", encoding="utf-8") as f:
            for ex in ds[split]:
                dialog = ex.get("dialog") or ex.get("utterances") or ex.get("text", [])
    ...
    with cleaned.open("w", encoding="utf-8") as cf:
        for fn in sorted(os.listdir(input_dir)):
            ...
            if dtype == "INSTRUCT":
                ...
                p = tmp_out
            # SFT and RLHF will be both merged; RLHF entries will be skipped later at tokenization
...
# Skips non-conversation lines so RLHF files can co-exist.
# -------------------------
def build_tokens(data_dir: str, save_dir: str, sp_prefix: str,
        ...
        with formatted.open("w", encoding="utf-8") as fout:
            for fn in sorted(data_dir.glob("train_shard_*.jsonl")):
                with fn.open("r", encoding="utf-8") as f:
                    for ln in f:
                        obj = json.loads(ln)
                        if "conversation" not in obj:
                            continue  # skip RLHF or malformed entries
    ...
    meta = {"vocab_size": sp.get_piece_size(), "max_seq_len": max_seq_len, "stride": stride}
...
# -------------------------
# WebDataset streaming dataset (optional)
# -------------------------
class WebTextDataset(IterableDataset):
    def __init__(self, url_pattern: str, sp: SentencePieceProcessor, max_len: int, pad_id: int):
        super().__init__()
        assert wds is not None, "webdataset library required"
    ...
    def __iter__(self):
        dataset = wds.WebDataset(self.url_pattern).decode()
        for sample in dataset:
            text = None
            for k in ("txt", "text", "json"):
                if k in sample:
                    text = sample[k]; break
            if text is None:
                continue
            ...
            if len(ids) < 2:
                x = torch.tensor([self.pad_id], dtype=torch.long)
                y = torch.tensor([-100], dtype=torch.long)
            else:
                arr = [self.sp.bos_id()] + ids + [self.sp.eos_id()]
                arr = torch.tensor(arr, dtype=torch.long)
                x = arr[:-1]; y = arr[1:].clone()
            yield x, y

# -------------------------
# Rotary embeddings and model
# -------------------------
...
def apply_rope(q: torch.Tensor, k: torch.Tensor, rope: RotaryEmbedding):
...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, d_model, n_heads, dropout, max_seq_len, use_rope=True):
        super().__init__()
        ...
        self.rope = RotaryEmbedding(self.d_head, max_seq_len) if use_rope else None
...
class Block(nn.Module):
    def __init__(self, d_model, n_heads, d_ff, dropout, max_seq_len, use_checkpoint=False):
        ...
        self.attn = MultiHeadSelfAttention(d_model, n_heads, dropout, max_seq_len, use_rope=True)
...
class GPTSmall(nn.Module):
    def __init__(self, vocab_size, d_model, n_layers, n_heads, d_ff, max_seq_len, dropout, use_checkpoint=False):
        ...
        self.pos_emb = nn.Embedding(max_seq_len, d_model)  # residual bias
        self.drop = nn.Dropout(dropout)
...
# -------------------------
# Generation utils (top-k / top-p)
# -------------------------
@torch.no_grad()
def sample(model: nn.Module, input_ids: torch.Tensor, max_new_tokens: int = 128, temperature: float = 0.8,
           top_k: Optional[int] = 50, top_p: Optional[float] = 0.9, eos_id: Optional[int] = None):
    model.eval()
    seq = input_ids
...
def ddp_or_fsdp_wrap(model: nn.Module, use_fsdp: bool, fsdp_min_params: int):
    if dist.is_available() and dist.is_initialized():
        if use_fsdp and FSDP_AVAILABLE:
            auto_wrap_policy = size_based_auto_wrap_policy(min_num_params=int(fsdp_min_params))
...
def deepspeed_config_from_cfg(cfg: Dict[str, Any]) -> Dict[str, Any]:
    amp_dtype = cfg.get("amp_dtype", "bf16").lower()
    fp16_enabled = (amp_dtype == "fp16")
    bf16_enabled = (amp_dtype == "bf16")
    return {
...
class PreferencePairDataset(Dataset):
    def __init__(self, jsonl_path: str, sp: SentencePieceProcessor, max_len: int):
        ...
        with open(jsonl_path, "r", encoding="utf-8") as f:
            for ln in f:
                obj = json.loads(ln)
                if not {"prompt","pos","neg"} <= obj.keys():
                    continue
                prompt = obj["prompt"]
                pos = obj["pos"]
                neg = obj["neg"]
...
def ppo_step(policy: GPTSmall, old_policy: GPTSmall, reward_model: RewardModel,
    ...
    lp_new = resp_logp_new.gather(-1, taken).squeeze(-1)
    ...
    return {"ppo_loss": float(loss.item()), "reward": float(rewards.mean().item()), "kl": float(kl.item()), "resp_lp": float(resp_lp.mean().item())}
...
def sft_train_loop(model: nn.Module, train_dl: DataLoader, val_dl: Optional[DataLoader],
                   ...
                   ema: EMA, optim: Optional[torch.optim.Optimizer], sched: Optional[CosineWithWarmup],
                   scaler: GradScaler, sp: SentencePieceProcessor, engine=None):
    ...
    if os.path.exists(model_path):
        ...
        if "optimizer" in ckpt and ckpt["optimizer"] is not None and engine is None and optim is not None:
            try: optim.load_state_dict(ckpt["optimizer"])
            except Exception: pass
        if "ema" in ckpt and ckpt["ema"] is not None:
            for k, v in ckpt["ema"].items(): ema.shadow[k] = v.to(device)
        step = ckpt.get("step", 0); best_val = ckpt.get("best_val", best_val)
    ...
    if wandb and config.get("use_wandb", False) and is_main_process():
        name = config.get("wandb_run_name") or f"run-{int(time.time())}"
        wandb.init(project=config.get("wandb_project", "gpt3_5_style_train"),
    ...
    use_bf16 = (config.get("amp_dtype", "bf16").lower() == "bf16") and torch.cuda.is_available()
    ...
    try:
        while step < total_steps:
            ...
            for X, Y in train_dl:
                ...
                with autocast(enabled=amp_enabled, dtype=torch.bfloat16 if use_bf16 else torch.float16):
                    logits = model(X) if engine is None else engine(X)
                    if engine is None:
                        loss = compute_loss(logits, Y_clone, ignore_index=-100) / float(grad_accum)
                    else:
                        loss = compute_loss(logits, Y_clone, ignore_index=-100)
                ...
                accum += 1
                if accum >= grad_accum or engine is not None:
                    if engine is None:
                        assert optim is not None
                    ...
                    if is_main_process():
                        if progress:
                            progress.update(1)
                            ploss = float(loss.item() * (grad_accum if engine is None else 1))
                            lr = None
                            if engine is None and optim is not None:
                                lr = optim.param_groups[0]["lr"]
                            elif engine is not None and hasattr(engine, "optimizer"):
                                lr = engine.optimizer.param_groups[0]["lr"]
                            progress.set_postfix({"loss": ploss, "lr": lr})
                        if writer and step % config["log_every"] == 0:
                            writer.add_scalar("train/loss", float(loss.item() * (grad_accum if engine is None else 1)), step)
                            if engine is None and optim is not None:
                                writer.add_scalar("train/lr", optim.param_groups[0]["lr"], step)
                            elif engine is not None and hasattr(engine, "optimizer"):
                                writer.add_scalar("train/lr", engine.optimizer.param_groups[0]["lr"], step)
                        if wandb and config.get("use_wandb", False) and step % config["log_every"] == 0:
                            log_lr = None
                            if engine is None and optim is not None:
                                log_lr = optim.param_groups[0]["lr"]
                            elif engine is not None and hasattr(engine, "optimizer"):
                                log_lr = engine.optimizer.param_groups[0]["lr"]
                            wandb.log({"train/loss": float(loss.item() * (grad_accum if engine is None else 1)),
                                       "train/lr": log_lr, "step": step})
                    ...
                    if is_main_process() and (step % config["save_every"] == 0 or step >= total_steps):
                        ckpt = {
                            ...
                            "optimizer": None if engine is not None else (optim.state_dict() if optim is not None else None),
...
# -------------------------
# RLHF training loop (Reward Model + PPO)
# -------------------------
def rlhf_train(tokens_dir: str, save_dir: str, config: Dict[str, Any], local_rank: Optional[int] = None):
    ...
    base_ckpt_path = os.path.join(save_dir, MODEL_FILENAME)
    if not os.path.exists(base_ckpt_path):
        if is_main_process():
            print("[WARN] Base SFT checkpoint not found. RLHF requires prior SFT model.")
        return
    base_ckpt = torch.load(base_ckpt_path, map_location=device)
    policy = GPTSmall(vocab_size, config["d_model"], config["n_layers"], config["n_heads"],
                      config["d_ff"], config["max_seq_len"], config["dropout"],
                      use_checkpoint=config.get("use_checkpoint", False)).to(device)
    policy.load_state_dict(base_ckpt["model"])
    old_policy = GPTSmall(vocab_size, config["d_model"], config["n_layers"], config["n_heads"],
                          config["d_ff"], config["max_seq_len"], config["dropout"], use_checkpoint=False).to(device)
    old_policy.load_state_dict(base_ckpt["model"]); old_policy.eval()

    rm_backbone = GPTSmall(vocab_size, config["d_model"], config["n_layers"], config["n_heads"],
                           config["d_ff"], config["max_seq_len"], config["dropout"], use_checkpoint=False).to(device)
    ...
    while step < config["total_steps"]:
        ...
        for prompts, pos_ids, neg_ids in pref_dl:
            ...
            if step % 100 == 0:
                old_policy.load_state_dict(policy.state_dict()); old_policy.eval()

            if is_main_process() and ppo_progress:
                ppo_progress.update(1); ppo_progress.set_postfix(stats)
...
def train(tokens_dir: str, save_dir: str, config: Dict[str, Any], local_rank: Optional[int] = None):
    ...
    use_webdataset = config.get("use_webdataset", False) and wds is not None and config.get("webdataset_url", "")
    if use_webdataset:
        train_ds = WebTextDataset(config["webdataset_url"], sp, max_len=config["max_seq_len"], pad_id=pad_id)
        train_dl = DataLoader(train_ds, batch_size=config["batch_size"], shuffle=False,
                              collate_fn=lambda b: collate_tokens(b, pad_id), num_workers=4, pin_memory=True)
        val_dl = None
    elif shard_ids_files:
        ...
        if val_ids.exists() and val_offs.exists():
            ...
            val_dl = DataLoader(val_ds, batch_size=config["batch_size"], shuffle=False,
                                collate_fn=lambda b: collate_tokens(b, pad_id), num_workers=2, pin_memory=True)
    else:
    ...
    model = GPTSmall(vocab_size, config["d_model"], config["n_layers"], config["n_heads"],
                     config["d_ff"], config["max_seq_len"], config["dropout"],
                     use_checkpoint=config.get("use_checkpoint", False)).to(device)

    engine = None
    if config.get("use_deepspeed", False) and DEEPSPEED_AVAILABLE and dist.is_initialized():
        ds_config = deepspeed_config_from_cfg(config)
        engine, optim, _, _ = deepspeed.initialize(model=model, model_parameters=model.parameters(), config=ds_config)
        model_wrapped = engine
    else:
        model_wrapped = ddp_or_fsdp_wrap(model, use_fsdp=config.get("use_fsdp", False), fsdp_min_params=int(config.get("fsdp_min_params", 1e6)))
        if bnb is not None:
            try:
                optim = bnb.optim.AdamW8bit(model_wrapped.parameters(), lr=config["lr"],
                                            betas=config["betas"], weight_decay=config["weight_decay"])
                if is_main_process(): print("[INFO] Using bitsandbytes AdamW8bit")
            except Exception:
                optim = torch.optim.AdamW(model_wrapped.parameters(), lr=config["lr"],
                                          betas=config["betas"], weight_decay=config["weight_decay"])
    ...
    sched = None if engine is not None else CosineWithWarmup(optim, config["warmup_steps"], config["total_steps"], base_lr=config["lr"])
    scaler = GradScaler(enabled=(device.type == "cuda"))
    ema = EMA(model.module if hasattr(model, "module") else model, decay=0.9999, device=device)

    sft_train_loop(model_wrapped if engine is None else model, train_dl, val_dl, device, config, save_dir, ema,
                   optim if engine is None else None, sched if engine is None else None, scaler, sp, engine=engine)
...
# -------------------------
def main():
    p = argparse.ArgumentParser(description="Industrial GPT-3.5-style training (SFT + optional RLHF)")
    ...
    sb = sub.add_parser("build_tokens", help="Train SentencePiece and tokenize to ids/offsets")
    ...
    st = sub.add_parser("train", help="Train GPT-3.5-style; multi-node via torchrun/deepspeed. Optional RLHF.")
    st.add_argument("--tokens_dir", default=TOKENS_DIR)
    st.add_argument("--save_dir", default=SAVE_DIR)
    ...
    st.add_argument("--amp_dtype", type=str, default=DEFAULT["amp_dtype"], choices=["fp16", "bf16"])
    st.add_argument("--use_fsdp", action="store_true")
    st.add_argument("--fsdp_min_params", type=float, default=DEFAULT["fsdp_min_params"])
    st.add_argument("--use_deepspeed", action="store_true")
    st.add_argument("--use_webdataset", action="store_true")
    st.add_argument("--webdataset_url", type=str, default=DEFAULT["webdataset_url"])
    # RLHF flags
    ...
    # Pipeline command to run everything end-to-end
    sa = sub.add_parser("all", help="Run full pipeline: preprocess -> build_tokens -> train")
    sa.add_argument("--input_dir", default=DATASET_DIR)
    sa.add_argument("--processed_dir", default=PROCESSED_DIR)
    sa.add_argument("--tokens_dir", default=TOKENS_DIR)
    sa.add_argument("--save_dir", default=SAVE_DIR)
    sa.add_argument("--vocab_size", type=int, default=DEFAULT_VOCAB_SIZE)
    sa.add_argument("--sp_prefix", default=SPM_PREFIX)
    sa.add_argument("--max_seq_len", type=int, default=DEFAULT_MAX_SEQ_LEN)
    sa.add_argument("--stride", type=int, default=DEFAULT_STRIDE)
    sa.add_argument("--rlhf_enabled", action="store_true")
    ...
    if args.cmd == "convert_hf":
        ...
        build_tokens(args.data_dir, args.save_dir, args.sp_prefix, args.vocab_size, args.max_seq_len, args.stride)
    elif args.cmd == "train":
        cfg = {
            "n_layers": DEFAULT["n_layers"], "n_heads": DEFAULT["n_heads"],
            "d_model": DEFAULT["d_model"], "d_ff": DEFAULT["d_ff"],
            "dropout": DEFAULT["dropout"], "max_seq_len": DEFAULT["max_seq_len"],
            "batch_size": args.batch_size, "grad_accum_steps": args.grad_accum_steps, "lr": args.lr,
            "weight_decay": DEFAULT["weight_decay"], "betas": DEFAULT["betas"], "warmup_steps": DEFAULT["warmup_steps"],
            "total_steps": args.total_steps, "save_every": args.save_every,
            "use_checkpoint": args.use_checkpoint, "seed": args.seed,
            "log_every": args.log_every, "eval_every": args.eval_every, "clip_grad": args.clip_grad,
            "use_wandb": args.use_wandb, "wandb_project": args.wandb_project, "wandb_run_name": args.wandb_run_name,
            "amp_dtype": args.amp_dtype, "use_fsdp": args.use_fsdp, "fsdp_min_params": args.fsdp_min_params,
            "use_deepspeed": args.use_deepspeed, "use_webdataset": args.use_webdataset, "webdataset_url": args.webdataset_url,
            # RLHF
            "rlhf_enabled": args.rlhf_enabled, "rm_lr": args.rm_lr, "ppo_lr": args.ppo_lr, "ppo_epochs": args.ppo_epochs,
            "ppo_batch_size": args.ppo_batch_size, "ppo_kl_coef": args.ppo_kl_coef, "ppo_clip": args.ppo_clip,
        ...
        # 1) Preprocess
        preprocess(args.input_dir, args.processed_dir, val_ratio=0.02, shard_size=200000)
        # 2) Build tokens
        build_tokens(args.processed_dir, args.tokens_dir, args.sp_prefix, args.vocab_size, args.max_seq_len, args.stride)
        # 3) Train (SFT; optionally RLHF)
        cfg = DEFAULT.copy()
        cfg["rlhf_enabled"] = args.rlhf_enabled
        local_rank = int(os.environ.get("LOCAL_RANK", 0)) if torch.cuda.is_available() else None
        train(args.tokens_dir, args.save_dir, cfg, local_rank)
